...
final parroquiaFutureProvider = FutureProvider.autoDispose.family<List<Parroquia>, int>((ref, idCanton) async {
  ...
  try {
    final response = await dio.get('${Env.clientApiUrl}/parroquia/canton/${idCanton}/all',
        options: Options(headers: {'Authorization': token}));

    if (response.statusCode == 200) {
      return (response.data as List).map((canton) => Parroquia.fromMap(canton)).toList();
    } else {
      throw Exception('Error al obtener las parroquias del camtón ${idCanton}. Error: ${response.data['error']}');
    }
  } catch (e) {
    throw Exception('Error al obtener las parroquias del camtón ${idCanton}..  Error: ${e.toString()}');
  }
});
