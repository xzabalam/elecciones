...
final cantonFutureProvider = FutureProvider.autoDispose.family<List<Canton>, int>((ref, idProvincia) async {
  ...
  try {
    final response = await dio.get('${Env.clientApiUrl}/canton/provincia/${idProvincia}/all',
        options: Options(headers: {'Authorization': token}));

    if (response.statusCode == 200) {
      return (response.data as List).map((canton) => Canton.fromMap(canton)).toList();
    } else {
      throw Exception('Error al obtener los cantones de la provincia ${idProvincia}. Error: ${response.data['error']}');
    }
  } catch (e) {
    throw Exception('Error al obtener los cantones de la provincia ${idProvincia}.  Error: ${e.toString()}');
  }
});
