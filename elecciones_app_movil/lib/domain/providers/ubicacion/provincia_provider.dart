...
final provinciaFutureProvider = FutureProvider.autoDispose.family<List<Provincia>, int>((ref, idPais) async {
  ...
  try {
    final response = await dio.get('${Env.clientApiUrl}/provincia/pais/${idPais}/all',
        options: Options(headers: {'Authorization': token}));

    if (response.statusCode == 200) {
      return (response.data as List).map((provincia) => Provincia.fromMap(provincia)).toList();
    } else {
      throw Exception('Error al obtener las provincias del país ${idPais}. Error: ${response.data['error']}');
    }
  } catch (e) {
    throw Exception('Error al obtener las provincias del país ${idPais}.  Error: ${e.toString()}');
  }
});
