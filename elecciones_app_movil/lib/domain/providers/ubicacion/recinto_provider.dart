...
final recintoFutureProvider = FutureProvider.autoDispose.family<List<Recinto>, int>((ref, idZona) async {
  ...
  try {
    final response = await dio.get('${Env.clientApiUrl}/recinto/zona/${idZona}/all',
        options: Options(headers: {'Authorization': token}));

    if (response.statusCode == 200) {
      return (response.data as List).map((recinto) => Recinto.fromMap(recinto)).toList();
    } else {
      throw Exception('Error al obtener los recintos de la zona ${idZona}. Error: ${response.data['error']}');
    }
  } catch (e) {
    throw Exception('Error al obtener los recintos de la zona ${idZona}..  Error: ${e.toString()}');
  }
});
