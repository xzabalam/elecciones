...
final zonaFutureProvider = FutureProvider.autoDispose
    .family<List<Zona>, int>((ref, idParroquia) async {
  String token = ref.read(authTokenProvider).basicToken;

  final Dio dio = new Dio();
  try {
    final response = await dio.get(
        '${Env.clientApiUrl}/zona/parroquia/${idParroquia}/all',
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          'Error al obtener las zonas de la parroquia ${idParroquia}. Error: ${response.data['error']}');
    }
  } catch (e) {
    throw Exception(
        'Error al obtener las zonas de la parroquia ${idParroquia}..  Error: ${e.toString()}');
  }
});
